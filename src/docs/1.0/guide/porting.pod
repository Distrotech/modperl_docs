...
through the L<Perl Reference|general::perl_reference::perl_reference>
...
do(), %INC and @INC Explained|general::perl_reference::perl_reference/use____require____do_____INC_and__INC_Explained>.
...
L</perl-status location|guide::debug/Apache__Status____Embedded_Interpreter_Status_Information>, fetch
...
Sidenote: I<Pumpkin> A humorous term for the token (notional or real)
...
instead of error_log|guide::snippets/Redirecting_Errors_to_the_Client_Instead_of_error_log>".
...
mod_perl|guide::performance/Forking_and_Executing_Subprocesses_from_mod_perl>"
section to learn about implications of forking.

Also read about
L<Apache::SubProcess|guide::modules/Apache__SubProcess> for overridden
system() and exec() implementations that work with mod_perl.
...
L<Apache::GTopLimit|guide::modules/Apache__GTopLimit___Limit_Apache_httpd_processes> (based on
...
and fully 
qualified|general::perl_reference::perl_reference/Variables_Globally__Lexically_Scoped_And_Fully_Qualified>" for more information.
...
secrets|guide::porting/Exposing_Apache__Registry_secrets> we have seen
that the code is wrapped in a C<handler> routine, effectively turning
any subroutines within the file a script resides in into nested
...
Pressed the Stop Button'|guide::debug/Handling_the__User_pressed_Stop_button__case>.
...
I<perlsec> manpage for more information. Also read the I<re> pragma's
manpage.)
...
wrote the L<Apache::LogSTDERR|guide::porting/CGI_to_mod_perl_Porting__mod_perl_Coding_guidelines__> module to work
around this.
...
automatically reloaded when they change on disk. See L<Reloading
Modules and Required
Files|guide::porting/Reloading_Modules_and_Required_Files> for more
information.
...
Since we use C<CGI.pm>, it relies on the fact that C<$r> was set in the
...
Performance|guide::performance/TMTOWTDI__Convenience_and_Habit_vs__Performance>"
