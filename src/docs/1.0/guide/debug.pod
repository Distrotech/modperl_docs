...
1.19_01.  As a matter of fact none of the C<local $SIG{FOO}> signals
restores the original C handler - read L<Debugging Signal Handlers
($SIG{FOO})|guide::debug/Debugging_Signal_Handlers___SIG_FOO__> for a
...
Modules|guide::modules/Apache__VMonitor____Visual_System_and_Apache_Server_Monitor>"
...
Please notice that this demonstration was done before perl 5.8 was
released, which redefined some of the letters to have a different
meaning. For example C<w> was replaced with C<v>. Please see the
C<perldebug> manpage.
...
and terminator|guide::modules/Apache__Watchdog__RunAway___Hanging_Processes_Monitor_and_Terminator>
...
L<Apache::Status|guide::debug/Apache__Status____Embedded_Interpreter_Status_Information>.  What we see at
