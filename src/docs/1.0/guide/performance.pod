...
The LWP::Parallel::UserAgent benchmark: F<code/lwp-bench.pl>
...
L<Apache::VMonitor|guide::debug/Apache__VMonitor____Visual_System_and_Apache_Server_Monitor>, so instead of
...
L<ApacheBench|guide::performance/Configuration_Tuning_with_ApacheBench> B<C<ab>> utility
...
A workaround for this might be to use an L<output
filter|guide::modules/Apache__Filter___Alter_the_output_of_previous_handlers>
...
L<C<Apache::Status>|guide::debug/Apache__Status____Embedded_Interpreter_Status_Information> module.
...
=head2 Avoid the Evil Troika

The I<perlre> manpage says:
...
The mere existence of these variables will trigger this behavior,
regardless of whether or not the code that accesses them will be
executed. Removing these variables should significantly improve the
regex performance.
...
and write new code. Luckily C<Devel::SawAmpersand> comes to help.
